-- ============================================================
-- ECOMMERCE PHASE 2 - PostgreSQL Table Creation + CSV Import
-- ============================================================
-- Usage:
--   1. Save your 9 CSV files in a folder (e.g. C:/data/ or /home/user/data/)
--   2. Edit the file paths below (search for 'PASTE_YOUR_PATH_HERE') to match your folder
--   3. Run this file with psql:
--        psql -U your_username -d your_database -f ecommerce_phase2_views.sql
--   Note: \copy runs from the psql CLIENT machine (works with your local file paths).
--   If using pgAdmin's Query Tool instead, use its Import/Export feature for the CSVs,
--   or replace \copy with COPY (server-side, requires the file to sit on the DB server).
-- ============================================================
...
CREATE TABLE marketing_campaigns (
    campaign_row_id               SERIAL PRIMARY KEY,
    date                            DATE,
    channel                          TEXT,
    campaign                          TEXT,
    spend                              NUMERIC(12,2),
...
-- ============================================================
-- DATA IMPORT (\copy runs from your local machine via the psql client)
-- Replace 'PASTE_YOUR_PATH_HERE' with the actual folder path where
-- your CSV files are saved on YOUR computer.
-- Example (Windows):  C:/Users/YourName/Downloads/
-- Example (Mac/Linux): /Users/yourname/Downloads/
-- ============================================================

\copy customers            FROM 'PASTE_YOUR_PATH_HERE/cl_customers.csv'            DELIMITER ',' CSV HEADER;
\copy products              FROM 'PASTE_YOUR_PATH_HERE/cl_products.csv'              DELIMITER ',' CSV HEADER;
\copy orders                 FROM 'PASTE_YOUR_PATH_HERE/cl_orders.csv'                 DELIMITER ',' CSV HEADER;
\copy order_items             FROM 'PASTE_YOUR_PATH_HERE/cl_order_items.csv'             DELIMITER ',' CSV HEADER;
\copy payments                 FROM 'PASTE_YOUR_PATH_HERE/cl_payments.csv'                 DELIMITER ',' CSV HEADER;
\copy returns                   FROM 'PASTE_YOUR_PATH_HERE/cl_returns.csv'                   DELIMITER ',' CSV HEADER;
\copy marketing_campaigns (date, channel, campaign, spend, impressions, clicks, conversions) FROM 'PASTE_YOUR_PATH_HERE/cl_marketing_campaigns.csv' DELIMITER ',' CSV HEADER;
\copy web_events                 FROM 'PASTE_YOUR_PATH_HERE/cl_web_events.csv'                 DELIMITER ',' CSV HEADER;
\copy experiments (experiment_id, customer_id, variant, experiment_date, converted, revenue) FROM 'PASTE_YOUR_PATH_HERE/cl_experiments.csv' DELIMITER ',' CSV HEADER;
...
CREATE INDEX idx_webevents_customer   ON web_events(customer_id);
CREATE INDEX idx_webevents_name       ON web_events(event_name);
CREATE INDEX idx_marketing_date       ON marketing_campaigns(date);
CREATE INDEX idx_experiments_customer ON experiments(customer_id);
CREATE INDEX idx_experiments_expid    ON experiments(experiment_id);
...
SELECT
    ...
    (CURRENT_DATE - last_order_date) AS days_since_last_order
FROM customer_spend
WHERE (CURRENT_DATE - last_order_date) > 90   -- churned
